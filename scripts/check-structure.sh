...
# 每次 git commit 前和 Phase 3 交付前自动运行
#
# 检查项：
#   1. apps/*/src/ 下无 hooks/ lib/ utils/ constants/ validators/ 目录
#   2. 纯工具函数文件（无 next/* import）不在 apps/ 下
#   3. app 目录命名符合规范
#   4. scripts/ 三件套完整（setup.sh check.sh deploy.sh）
#   5. 每个 app 的 package.json 引用了 shared workspace
#   6. shared/ 导出覆盖所有子目录
...
while IFS= read -r f; do
  if ! grep -q "from 'next/" "$f" 2>/dev/null && ! grep -q "from \"next/" "$f" 2>/dev/null; then
    # Check if it's React-specific (uses 'use client' and React hooks — those are OK in components)
...
# ── 6. shared/ package.json exports 覆盖所有子目录 ──────────────
echo ""
echo "[6/6] shared/ 导出完整性检查"
# Use Python to parse JSON exports properly
...
if [ "$HAS_WILDCARD" = "YES" ]; then
  ...
  echo "  ⚠️  shared/ 无通配符导出，部分子目录可能不可达"
fi
